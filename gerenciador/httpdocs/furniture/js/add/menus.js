data_menus_json.template = '<div id="tr_#ID#" class="row_js#CLASS#"#ADD#>';
data_menus_json.template += '  <div class="cell" style="height: 45px;min-width: 70px !important;overflow:hidden;"#ADD_NAME#">#NAME#</div>';
data_menus_json.template += '  <div class="cell" style="min-width: 70px !important;overflow:hidden;"#ADD_PERFIL#">#PERFIL#</div>';
data_menus_json.template += '  <div class="cell" style="min-width: 70px !important;overflow:hidden;"#ADD_UPDATE#">#UPDATE#</div>';
data_menus_json.template += '  <div class="cell cell_last" style="min-width: 70px !important;overflow:hidden;">#ACAO#</div>';
data_menus_json.template += '</div>';

data_menus_json.footer = '<div class="row_js table_data_footer">';
data_menus_json.footer += ' <div class="cell cell_last table_data_footer">';
data_menus_json.footer += '     Linha por Página ';
data_menus_json.footer += '     <select id="per_page">';
data_menus_json.footer += '         <option value="20" selected>20</option>';
data_menus_json.footer += '         <option value="50">50</option>';
data_menus_json.footer += '         <option value="100">100</option>';
data_menus_json.footer += '     </select>';
data_menus_json.footer += ' </div>';
data_menus_json.footer += ' <div class="cell cell_last table_data_footer" style="justify-content: space-around;" id="paginate_control"></div>';
data_menus_json.footer += ' <div class="cell cell_last table_data_footer" id="paginate_display">#DATA_TOTAL#</div>';
data_menus_json.footer += '</div>';
...
$.ajax({
    type: "GET",
    url: data_menus_json.url
    , data: data_menus_json.data
    ,success: function( data ){
        $.each( data.total , function(i,o){
            $( "#" + i + "SpanUser").html( o )
        })
        data_menus_json.out_data = data ;
        fn_common.render_data( 
            data_menus_json.template + '<div class="row_data"></div>'
            , {
                "#CLASS#" : " row_js_header"
                , "#ADD#": ""
        ...
        $.each( data.row , function(i,o){
            ...
            fn_common.render_data( 
                ...
                , {
                    ...
                    , "#ACAO#" : '<a href="' + String( data_menus_json.action ).replace(/%d/,o.idx)  + '">Editar<i class="fa fa-edit"></i></a><a id="btn_remove_' + o.idx + '" href="' + String( data_menus_json.action ).replace(/%d/, o.idx) + '"> Excluir <i class="fa fa-trash"></i></a>'
                }
                , "#solaris-head-data .row_data" 
            ) ;

            $("a[id^='btn_remove_" + o.idx + "']").bind("click", function(){ 
                var target = $(this);
...
var fn_common = {
    ...
    paginate: function( sr ){
        paginate_control_button = '<button type="button" id="btn_#ID#" class="btn">#menu#</button>';
        ...
        if( paginate < data_menus_json.out_data.total.total ){
            last_paginate = data_menus_json.out_data.total.total % per_page == 0 ? data_menus_json.out_data.total.total / per_page : ( Math.floor( data_menus_json.out_data.total.total / per_page ) + 1 ) ;
        ...
        paginate_text += ' de ' + data_menus_json.out_data.total.total;
