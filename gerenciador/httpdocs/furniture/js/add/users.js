...
var fn_common = {
    ...
    paginate: function( sr ){
        ...
        if( paginate < data_users_json.out_data.total.total ){
            last_paginate = data_users_json.out_data.total.total % per_page == 0 ? data_users_json.out_data.total.total / per_page : ( Math.floor( data_users_json.out_data.total.total / per_page ) + 1 ) ;
...
$(document).ready( function(){
    /*$.each(['name','user','grupo','mail','cpf','status'], function(i,o){
        $("#btn_ordenation_" + o ).bind("click",function(){
            fn_common.ordenation( o )
        })
    });*/
    fn_common.paginate( data_users_json.page );
