data_categorias_json.template = '<div class="row_js#CLASS#"#ADD#>';
data_categorias_json.template += '  <div class="cell" style="height: 45px;min-width: 70px !important;overflow:hidden;"#ADD_NAME#>#NAME#</div>';
data_categorias_json.template += '  <div class="cell cell_last" style="height: 45px;text-align:center !important;min-width: 70px !important;overflow:hidden;"#ADD_ACAO#>#ACAO#</div>';
data_categorias_json.template += '</div>';

data_categorias_json.footer = '<div class="row_js table_data_footer">';
data_categorias_json.footer += ' <div class="cell cell_last table_data_footer">';
data_categorias_json.footer += '     Linha por Página ';
data_categorias_json.footer += '     <select id="per_page">';
data_categorias_json.footer += '         <option value="20" selected>20</option>';
data_categorias_json.footer += '         <option value="50">50</option>';
data_categorias_json.footer += '         <option value="100">100</option>';
data_categorias_json.footer += '     </select>';
data_categorias_json.footer += ' </div>';
data_categorias_json.footer += ' <div class="cell cell_last table_data_footer" style="justify-content: space-around;" id="paginate_control"></div>';
data_categorias_json.footer += ' <div class="cell cell_last table_data_footer" id="paginate_display">#DATA_TOTAL#</div>';
data_categorias_json.footer += '</div>';

$.ajax({
    type: "GET",
    url: data_categorias_json.url
    , data: data_categorias_json.data
    ,success: function( data ){
        data_categorias_json.out_data = data ;
        fn_common.render_data( 
            data_categorias_json.template + '<div class="row_data"></div>'
            , {
                "#CLASS#" : " row_js_header"
                , "#ADD#": ""
        ...
        $.each( data.row , function(i,o){
            fn_common.render_data( 
                ...
                , {
                    ...
                    , "#NAME#" :   o.title 
                    , "#ADD_ACAO#" : ''
                    , "#ACAO#" :  '<a href="' + String( data_categorias_json.action ).replace('%d', o.idx) + '" class="btn button info round"><i class="fontello-edit"></i> Editar</a><a id="btn_remove_' + o.idx + '" href="' + String( data_categorias_json.action ).replace('%d', o.idx) + '" class="btn button alert round"><i class="fontello-trash"></i> Excluir </a>'
                }
...
var fn_common = {
    ...
    paginate: function( sr ){
        ...
        if( paginate < data_categorias_json.out_data.total.total ){
            last_paginate = data_categorias_json.out_data.total.total % per_page == 0 ? data_categorias_json.out_data.total.total / per_page : ( Math.floor( data_categorias_json.out_data.total.total / per_page ) + 1 ) ;
        ...
        paginate_text += ' de ' + data_categorias_json.out_data.total.total;
