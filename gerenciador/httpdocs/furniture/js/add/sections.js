data_sections_json.template = '<div class="row_js#CLASS#"#ADD#>';
data_sections_json.template += '  <div class="cell" style="height: 45px;min-width: 70px !important;overflow:hidden;"#ADD_NAME#">#NAME#</div>';
data_sections_json.template += '  <div class="cell" style="min-width: 70px !important;overflow:hidden;"#ADD_UPDATE#">#UPDATE#</div>';
data_sections_json.template += '  <div class="cell cell_last" style="min-width: 70px !important;overflow:hidden;">#ACAO#</div>';
data_sections_json.template += '</div>';

data_sections_json.footer = '<div class="row_js table_data_footer">';
data_sections_json.footer += ' <div class="cell cell_last table_data_footer">';
data_sections_json.footer += '     Linha por Página ';
data_sections_json.footer += '     <select id="per_page">';
data_sections_json.footer += '         <option value="20" selected>20</option>';
data_sections_json.footer += '         <option value="50">50</option>';
data_sections_json.footer += '         <option value="100">100</option>';
data_sections_json.footer += '     </select>';
data_sections_json.footer += ' </div>';
data_sections_json.footer += ' <div class="cell cell_last table_data_footer" style="justify-content: space-around;" id="paginate_control"></div>';
data_sections_json.footer += ' <div class="cell cell_last table_data_footer" id="paginate_display">#DATA_TOTAL#</div>';
data_sections_json.footer += '</div>';
...
$.ajax({
    type: "GET",
    url: data_sections_json.url
    , data: data_sections_json.data
    ,success: function( data ){
        $.each( data.total , function(i,o){
            $( "#" + i + "SpanUser").html( o )
        })
        data_sections_json.out_data = data ;
        fn_common.render_data( 
            data_sections_json.template + '<div class="row_data"></div>'
            , {
                "#CLASS#" : " row_js_header"
                , "#ADD#": ""
        ...
        $.each( data.row , function(i,o){
            ...
            fn_common.render_data( 
                ...
                , {
                    ...
                    , "#ADD_UPDATE#": ''
                    , "#NAME#" :  o.name
        		    , "#STATUS#" : data_sections_json.status_published_list[ o.new_status ] != undefined ? data_sections_json.status_published_list[ o.new_status ]  : "-"
        		    , "#UPDATE#" : o.modified_at != null ? String( o.modified_at ).replace(/^(....).(..).(..).(.....).+/,"$3/$2/$1 $4" ) : String( o.created_at ).replace("/^(....).(..).(..).(.....).+/","$3/$2/$1 $4" )
                    , "#ACAO#" : '<a href="' + String( data_sections_json.url_edit ).replace(/%s/,o.idx)  + '" style="width: 100%; background-color: #ffffff; color: #707070; text-align: left; font-weight: bold; padding: 0px;" id="btn_ordenation_status" type="button">Editar<i class="fa fa-edit"></i></a>'
                }
...
var fn_common = {
    ...
    paginate: function( sr ){
        ...
        if( paginate < data_sections_json.out_data.total.total ){
            last_paginate = data_sections_json.out_data.total.total % per_page == 0 ? data_sections_json.out_data.total.total / per_page : ( Math.floor( data_sections_json.out_data.total.total / per_page ) + 1 ) ;
        ...
        paginate_text += ' de ' + data_sections_json.out_data.total.total;
