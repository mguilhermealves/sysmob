data_agendas_json.template = '<div class="row_js#CLASS#"#ADD#>';
data_agendas_json.template += '  <div class="cell" style="height: 45px;min-width: 70px !important;overflow:hidden;"#ADD_NAME#>#NAME#</div>';
data_agendas_json.template += '  <div class="cell cell_last" style="height: 45px;text-align:center !important;min-width: 70px !important;overflow:hidden;"#ADD_ACAO#>#ACAO#</div>';
data_agendas_json.template += '</div>';

data_agendas_json.footer = '<div class="row_js table_data_footer">';
data_agendas_json.footer += ' <div class="cell cell_last table_data_footer">';
data_agendas_json.footer += '     Linha por Página ';
data_agendas_json.footer += '     <select id="per_page">';
data_agendas_json.footer += '         <option value="20" selected>20</option>';
data_agendas_json.footer += '         <option value="50">50</option>';
data_agendas_json.footer += '         <option value="100">100</option>';
data_agendas_json.footer += '     </select>';
data_agendas_json.footer += ' </div>';
data_agendas_json.footer += ' <div class="cell cell_last table_data_footer" style="justify-content: space-around;" id="paginate_control"></div>';
data_agendas_json.footer += ' <div class="cell cell_last table_data_footer" id="paginate_display">#DATA_TOTAL#</div>';
data_agendas_json.footer += '</div>';

$.ajax({
    type: "GET",
    url: data_agendas_json.url
    , data: data_agendas_json.data
    ,success: function( data ){
        $.each( data.total , function(i,o){
            $( "#" + i + "SpanUser").html( o )
        })
        data_agendas_json.out_data = data ;
        fn_common.render_data( 
            data_agendas_json.template + '<div class="row_data"></div>'
            , {
                "#CLASS#" : " row_js_header"
                , "#ADD#": ""
        ...
        $.each( data.row , function(i,o){
            fn_common.render_data( 
                ...
                , {
                    ...
                    , "#NAME#" :   o.title 
                    , "#ADD_ACAO#" : ''
                    , "#ACAO#" :  '<a href="' + String( data_agendas_json.action ).replace('%d', o.idx) + '" class="btn button info round"><i class="fontello-edit"></i> Editar</a><a id="btn_remove_' + o.idx + '" href="' + String( data_agendas_json.action ).replace('%d', o.idx) + '" class="btn button alert round"><i class="fontello-trash"></i> Excluir </a>'
                }
...
var fn_common = {
    ...
    paginate: function( sr ){
        ...
        if( paginate < data_agendas_json.out_data.total.total ){
            last_paginate = data_agendas_json.out_data.total.total % per_page == 0 ? data_agendas_json.out_data.total.total / per_page : ( Math.floor( data_agendas_json.out_data.total.total / per_page ) + 1 ) ;
        ...
        paginate_text += ' de ' + data_agendas_json.out_data.total.total;
